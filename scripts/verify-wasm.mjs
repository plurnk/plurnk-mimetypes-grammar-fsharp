#!/usr/bin/env node
// CI byte-identical reproducibility check for fsharp.wasm.
...
const committed = await readFile(path.join(repoRoot, "fsharp.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed fsharp.wasm sha256: ${committedHash}`);

await using temporary = await mkdtempDisposable(path.join(tmpdir(), "grammar-fsharp-verify-"));
...
console.log(`rebuilt fsharp.wasm sha256: ${rebuiltHash}`);
