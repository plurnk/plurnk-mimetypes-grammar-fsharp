#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-fsharp.
...
await using temporary = await mkdtempDisposable(path.join(tmpdir(), "grammar-fsharp-build-"));
...
const buildCwd = path.join(work, "src", "fsharp");
await run(cli, ["generate"], { cwd: buildCwd });
await run(cli, ["build", "--wasm"], { cwd: buildCwd });

// Locate produced wasm and copy as fsharp.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "fsharp.wasm"));
const bytes = (await readFile(path.join(repoRoot, "fsharp.wasm"))).length;
console.log(`fsharp.wasm: ${bytes} bytes (built from ${pin})`);
